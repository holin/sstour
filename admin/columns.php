...
if($option == 'index')
{
	if($_POST['update'] == 'update')
	{
		...
		if(is_array($_POST['type_id']))
		{
			...
			{
				$GETSQL->fUpdate("`{$ODBC['tablepre']}columns`","`type_subject`='{$_POST['type_subject'][$v]}',`type_live`='{$_POST['type_live'][$v]}',`type_sp`='{$_POST['type_sp'][$v]}'","`type_id`='{$v}' AND `type_id`!='{$_POST['type_live'][$v]}'");
			}
		}
		$sql_class = $GETSQL->fSql("*","`{$ODBC['tablepre']}columns`","","ORDER BY `type_sp`,`type_id` DESC");
		foreach ($sql_class AS $vale)
		{
			$configtxt .= "columnstf[{$vale['type_id']}]='{$vale['type_live']}';columnsvf[{$vale['type_id']}]='{$vale['type_subject']}';\n";
		}
		ffile(R_P."lang/columns.js",$configtxt);
		die(gb2utf8("后台组件操作完成"));
	}
	$sql_about = $GETSQL->fSql("*","`{$ODBC['tablepre']}columns`","","ORDER BY `type_sp`,`type_id` DESC");
...
if($option == 'del')
{
	$sql_class = $GETSQL->fSql("*","`{$ODBC['tablepre']}columns`","`type_live`='{$id}'","","","","U_B");
	if($sql_class['type_id']>0)
	{
		die(gb2utf8("请先删除分类下的子类"));
	}else{
		$GETSQL->fDelete("`{$ODBC['tablepre']}columns`","`type_id`='{$id}'","1");
		die(gb2utf8("删除成功"));
	}
	//$GETSQL->fDelete("`{$ODBC['tablepre']}columns`","`type_id`='{$id}'","1");
	//die(gb2utf8("删除成功"));
