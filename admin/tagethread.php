...
if($option == 'index')
{
	...
	$nNums = $GETSQL->fNumrows("SELECT tag_id FROM `{$ODBC['tablepre']}hotelthreadtag` WHERE {$where}");
	if($nNums > 0)
	{
		$sql_comtag = $GETSQL->fSql("*","`{$ODBC['tablepre']}hotelthreadtag`","{$where}","ORDER BY `tag_num` DESC,`tag_id` DESC",$nPage*$nCount,$nCount);
...
if($option == 'del')
{
	...
	{
		$GETSQL->fDelete("`{$ODBC['tablepre']}hotelthreadtag`","`tag_id`='{$_POST['del'][$key]}'");
	}
	die(gb2utf8("É¾³ý³É¹¦"));
}
?>