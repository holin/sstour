...
if($option == 'index')
{
	...
	{
		$GETSQL->fDelete("`{$ODBC['tablepre']}statistics`","`st_admin`='0' AND TO_DAYS(`st_date`)<=TO_DAYS(NOW())-7");
		die(gb2utf8("É¾³ý³É¹¦"));
	}
	include_once Getincludefun("page");
	$cParameter = "action=statistics&Keyword={$Keyword}&id={$id}&type={$type}&Industry={$Industry}";
	$nCount = 20;
	if($Keyword!='')
	{
	...
	$nNums = $GETSQL->fNumrows("SELECT st_id FROM `{$ODBC['tablepre']}statistics` WHERE `st_admin`='0'{$where}");
	$sql_statistics = $GETSQL->fSql("*","`{$ODBC['tablepre']}statistics`","`st_admin`='0'{$where}","ORDER BY `st_date` DESC,`st_id` DESC",$nPage*$nCount,$nCount);
